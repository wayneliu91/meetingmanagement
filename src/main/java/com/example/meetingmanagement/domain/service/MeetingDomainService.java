...
public class MeetingDomainService {
        ...
        for (Meeting meeting : allMeetings) {
            if (!meeting.isHosted()) {
                ...
                for (Meeting attended : attendedMeetings) {
                    if (meeting.getStartTime().isBefore(attended.getEndTime()) && meeting.getEndTime().isAfter(attended.getStartTime())) {
                        conflict = true;
                        break;
                    }
                }
    ...
    public boolean isRoomAvailable(Room room, LocalDateTime startTime, LocalDateTime endTime, List<Meeting> existingMeetings) {
        for (Meeting meeting : existingMeetings) {
            if (meeting.getRoomId().equals(room.getId()) &&
                    ((startTime.isBefore(meeting.getEndTime()) && endTime.isAfter(meeting.getStartTime())))) {
                return false;
            }
        }
        return true;
